...
The performance of different algorithms is presented in the figures on the left. @fig-macro-recall presents the performance of all languages using the macro-recall. The best-performing system in almost all cases is DialectId, which is trained on 2 million tweets and has a vocabulary of 500,000 tokens. The exception is Turkish, where the best systems are trained with only 262k tweets.
